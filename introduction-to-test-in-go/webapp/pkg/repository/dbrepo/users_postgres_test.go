...
func TestMain(m *testing.M) {
	// connect to docker; fail if docker not runnig
	...
	opts := dockertest.RunOptions{
		...
		ExposedPorts: []string{"5432"}, // internal port
...
func TestPostgresDBRepoAllUsers(t *testing.T) {
	...
	if len(users) != 1 {
		t.Errorf("all users report wrong size; expected 1, but got: %d", len(users))
	}
	...
	if len(users) != 2 {
		t.Errorf("all users report wrong size; expected 2, but got: %d", len(users))
	}
}
...
func TestPostgresDBRepoGetUser(t *testing.T) {
	...
	if user.Email != "admin@example.com" {
		t.Errorf("wrong email returned by Getuser; expected admin@examples.com, but got: %s", user.Email)
	}
...
func TestPostgresDBRepoGetUserByEmail(t *testing.T) {
	...
	if user.ID != 2 {
		t.Errorf("wrong email returned by Getuser; expected 2, but got: %d", user.ID)
	}
}
